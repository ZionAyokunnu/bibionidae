    ...
    if config.get('enable_micro_inversions', False):
        ...
        for _, block in synteny_df.iterrows():
            block_genes = ortholog_df[
                ...
                (orthlog_df['second_chr'] == block['second_chr'])
...
if __name__ == "__main__":
    ...
    logger.info("Starting Complete Enhanced Synteny and Inversion Analyzer")
    
    try:
        # Run complete analysis
        results = run_complete_enhanced_analysis(COMPLETE_ENHANCED_CONFIG)
        ...
        enabled_features = [k for k, v in COMPLETE_ENHANCED_CONFIG.items() 
                          if k.startswith('enable_') and v]
        print(f"  Total enabled features: {len(enabled_features)}")
        for category in ['BUSCO', 'Ortholog', 'Synteny', 'Rearrangement', 'Inversion', 'Quality']:
            category_features = [f for f in enabled_features if category.lower() in f.lower()]
            if category_features:
                print(f"  {category} enhancements: {len(category_features)}")
        ...
        print(f"  Reports: {results['output_dir']}/reports/")
        
        print(f"\nAdvanced Capabilities:")
        print(f"  ✓ Adaptive parameter adjustment based on assembly quality")
        print(f"  ✓ Comprehensive paralog detection and handling")
        print(f"  ✓ Enhanced sequence alignment for ortholog detection")
        print(f"  ✓ Confidence scoring for all analyses")
        print(f"  ✓ Single-gene and micro-inversion detection")
        print(f"  ✓ Translocation and complex rearrangement analysis")
        print(f"  ✓ Statistical validation and error propagation")
        print(f"  ✓ Comprehensive quality assessment and reporting")
        
    except Exception as e:
        logger.error(f"Complete analysis failed: {str(e)}")
        print(f"\nError: {str(e)}")
        if COMPLETE_ENHANCED_CONFIG.get('enable_debug_output', False):
            import traceback
            traceback.print_exc()
